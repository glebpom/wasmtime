...
mod far_x86_memory {
    ...
    use cranelift_jit::{BranchProtection, JITMemoryProvider};
    ...
    impl JITMemoryProvider for FarMemoryProvider {
        fn allocate_readexec(&mut self, size: usize, align: u64) -> io::Result<*mut u8> {
            ...
            let placement = self
                .executable_placements
                .pop_front()
                .expect("placement for every executable allocation");
            self.allocate(size, align, placement, FinalProtection::ReadExecute)
        }
        ...
        fn allocate_readwrite(&mut self, size: usize, align: u64) -> io::Result<*mut u8> {
            ...
            self.allocate(size, align, placement, FinalProtection::ReadWrite)
        }
        ...
        fn allocate_readonly(&mut self, size: usize, align: u64) -> io::Result<*mut u8> {
            ...
            self.allocate(size, align, placement, FinalProtection::ReadOnly)
        }
...
/// anonymous read-only data object allocated more than ±2 GiB away.
#[cfg(all(target_arch = "x86_64", target_os = "linux"))]
#[test]
fn far_x86_data_address_uses_absolute_relocation() {
    use cranelift_codegen::binemit::Reloc;
    ...
    let mut func_ctx = FunctionBuilderContext::new();
    {
        ...
        let address = bcx
            .ins()
            .global_value(module.target_config().pointer_type(), data_ref);
    ...
    assert_eq!(relocs[0].kind, Reloc::Abs8);
...
#[cfg(all(target_arch = "x86_64", target_os = "linux"))]
#[test]
fn far_x86_function_address_uses_absolute_relocation() {
    use cranelift_codegen::binemit::Reloc;
    ...
    assert_eq!(
        relocs
            .iter()
            .filter(|reloc| reloc.kind == Reloc::Abs8)
            .count(),
        1
    );
